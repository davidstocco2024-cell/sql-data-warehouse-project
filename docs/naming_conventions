...
These rules ensure **consistency, readability, scalability, and governance** across all data layers — **Bronze, Silver, and Gold**.
...
**Example:**  
`crm_customer_info` → Raw CRM data for customer information.
...
**Example:**  
`crm_customer_info` → Cleaned and standardized CRM customer table.
...
- `report_sales_monthly` → Pre-aggregated monthly report.
...
- `dwh_updated_by` → Process or user that last updated the record.  
...
- `<entity>`: Optional — specific entity or dataset.  

**Examples:**  
- `load_bronze_crm_customers` → Ingests raw CRM data.  
- `load_silver_crm_customers` → Transforms CRM data into Silver layer.  
- `load_gold_fact_sales` → Loads curated sales fact data.  
...
- Use **self-descriptive** names — avoid cryptic abbreviations.  
- Prefix temporary or staging tables consistently:  
- `tmp_` → Temporary processing  
- `stg_` → Staging  
- `bkp_` → Backup  
